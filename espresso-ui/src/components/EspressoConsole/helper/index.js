...
export function jsonToPhpArray(json) {
    ...
    function convert(obj) {
        if (Array.isArray(obj)) {
            return 'array(' + obj.map(convert).join(', ') + ')';
        } else if (typeof obj === 'object') {
            let entries = Object.entries(obj).map(([key, value]) => `'${key}' => ${convert(value)}`);
...
export function jsonToGo(json, typename, flatten = true, example = false, allOmitempty = false) {
    ...
    function findBestValueForNumberType(existingValue, newValue) {
        ...
        if (newGoType.includes('int') && existingGoType.includes('int')) {
            ...
            if (!isFinite(existingValueAbs + newValueAbs)) return Number.MAX_SAFE_INTEGER;
...
export function jsonToGoMap(json, varName = 'data') {
    if (typeof json === 'string') json = JSON.parse(json);
    else if (typeof json !== 'object' || json === null) {
        throw new Error('Input must be a valid JSON object or array.');
    }
    function convert(obj, indent = 1) {
        ...
        let entries = Object.entries(obj).map(([key, value]) => {
            ...
            if (Array.isArray(value)) {
                ...
                formattedValue += value.map(v => convertValue(v, indent + 1)).join(', ');
                formattedValue += '}';
            } else if (typeof value === 'object' && value !== null) {
                formattedValue = `map[string]interface{}{\n${convert(value, indent + 1)}\n${indentation}}`;
            } else {
                formattedValue = convertValue(value, indent);
            }

            return `${indentation}${formattedKey}: ${formattedValue},`;
        });
...
export function jsonToGoMapInterface(json, varName = 'data') {
    if (typeof json === 'string') json = JSON.parse(json);
    else if (typeof json !== 'object' || json === null) {
        throw new Error('Input must be a valid JSON object or array.');
    }
...
export function getHtmlFromHtmlTemplate(htmlTemplate, jsonData) {
    // Replace the placeholders for simple and nested values (e.g., {{.page_heading}} or {{.user.name}})
    htmlTemplate = htmlTemplate.replace(/{{\.(\w+(?:\.\w+)*)}}/g, function (match, key) {
        const value = key.split('.').reduce((obj, k) => (obj && obj[k] !== undefined ? obj[k] : undefined), jsonData);
        return value !== undefined ? value : match;
    });

    // Handle the range loop for arrays (e.g., {{range .QRTickets}} ... {{end}})
    htmlTemplate = htmlTemplate.replace(/{{range\s+\.(\w+)}}([\s\S]*?){{end}}/g, function (match, arrayKey, content) {
        if (jsonData[arrayKey] && Array.isArray(jsonData[arrayKey])) {
            return jsonData[arrayKey]
                .map(item => {
                    return content.replace(/{{\.(\w+(?:\.\w+)*)}}/g, (innerMatch, innerKey) => {
                        const innerValue = innerKey
                            .split('.')
                            .reduce((obj, k) => (obj && obj[k] !== undefined ? obj[k] : undefined), item);
                        return innerValue !== undefined ? innerValue : innerMatch;
                    });
                })
                .join('');
        }
